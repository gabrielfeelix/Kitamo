        ...
        <Screen bg={KITAMO.bg} edges={['top']}>
            <ScrollView contentContainerStyle={{ paddingBottom: bottomPad + 80 }} showsVerticalScrollIndicator={false}>
                <View style={{ paddingHorizontal: 22, paddingTop: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    <Text style={{ fontSize: 26, fontWeight: '800', letterSpacing: -0.6, color: KITAMO.ink }}>Minhas contas</Text>
                    <Pressable
    ...
    if (list.length === 0) {
        return (
            <View style={{ paddingVertical: 60, alignItems: 'center' }}>
                <Text style={{ fontSize: 14, color: KITAMO.muted, textAlign: 'center' }}>
            ...
            {list.map((c) => (
                <View
                    ...
                    style={{
                        ...
                        height: 180,
                        borderRadius: 20,
                        backgroundColor: c.color ?? '#5B0286',
                        overflow: 'hidden',
                        justifyContent: 'space-between',
                    }}
                >
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        <Text style={{ fontSize: 13, fontWeight: '700', color: '#fff', opacity: 0.9 }}>{c.name}</Text>
                        <Icon name="card" size={24} color="#fff" />
                    </View>
                    <View>
                        <Text style={{ fontSize: 11, color: '#fff', opacity: 0.8 }}>Limite</Text>
                        <Text style={{ fontSize: 28, fontWeight: '800', color: '#fff', letterSpacing: -0.5 }}>
                            R$ {(c.credit_limit ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </Text>
                    </View>
                </View>
            ))}
